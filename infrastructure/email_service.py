...
def send_device_otp_email(to_email: str, first_name: str, otp_code: str, device_name: str = "New Device") -> bool:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Initial Resend dispatch error for {to_email}: {err_msg}")
            fallback_recipient = "kranthikumarss28@gmail.com"
            print(f"⚠️ [Resend Fallback] Sending testing OTP for {to_email} to verified owner email: {fallback_recipient}", flush=True)
            try:
                res_fallback = resend.Emails.send({
                    "from": from_email,
                    "to": fallback_recipient,
                    "subject": f"LifeFlow Verification Code: {otp_code} (For: {to_email})",
                    "html": html_content
                })
